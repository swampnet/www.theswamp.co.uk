...
public class ChatHub : Hub
{
	...
	public ChatHub(IChatService chatService, IConnectionTracker connectionTracker, ILogger<ChatHub> logger)
	{
	...
	public override async Task OnConnectedAsync()
	{
		...
		_logger.LogInformation(
			"Chat connected: {DisplayName} ({ConnectionId}) from {IpAddress}",
			displayName, Context.ConnectionId, ipAddress);
	...
	public override async Task OnDisconnectedAsync(Exception? exception)
	{
		...
		if (exception is not null)
		{
			_logger.LogWarning(exception,
				"Chat disconnected with error: {DisplayName} ({ConnectionId})",
				displayName, Context.ConnectionId);
		}
		else
		{
			_logger.LogInformation(
				"Chat disconnected: {DisplayName} ({ConnectionId})",
				displayName, Context.ConnectionId);
		}
